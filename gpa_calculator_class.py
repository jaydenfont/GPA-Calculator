...
class Calculator:
    ...
    def calculate_button(g1, c1, g2, c2, g3, c3, g4, c4, g5, c5):
        # used for GUI
        total_pts = (Calculator.letter_values.get(g1) * c1) + (Calculator.letter_values.get(g2) * c2) + (Calculator.letter_values.get(g3) * c3) + (Calculator.letter_values.get(g4) * c4) + (Calculator.letter_values.get(g5) * c5)
    ...
    def calculate_obj(self): # will work with values passed into object, not done
        ...
        total_pts = (Calculator.letter_values.get(self.g1) * self.c1) + (Calculator.letter_values.get(self.g2) * self.c2) + (Calculator.letter_values.get(self.g3) * self.c3) + (Calculator.letter_values.get(self.g4) * self.c4) + (Calculator.letter_values.get(self.g5) * self.c5)
    ...
    def cumulative(total_credits, total_pts): #parameters from .round()
        ...
        if cumulative_prompt == 'Y' or cumulative_prompt == 'y':
            ...
            if isJay == Calculator.buID:
                ...
                print('Note: must update honor points and credits after each semester.\nSee "Classes" Tab under Student Link')
